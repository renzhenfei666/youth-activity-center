<template>
  <!-- 如果当前 subroute 有子节点 -->
  <el-submenu v-if="subroute.children && subroute.children.length > 0"
    :index="genPath(fatherpath, subroute.pcOpenUrl)">
    <!-- 创建菜单分组 -->
    <template slot="title">
      <i :class="subroute.pcIcon"></i>
    ...
    <SidebarItem v-for="(submenu, subidx) in subroute.children"
      :subroute="submenu"
      :fatherpath="genPath(fatherpath, subroute.pcOpenUrl)"
      :barIdx="subidx" 
      :key="barIdx + '-' + subidx" 
      />
  </el-submenu>
