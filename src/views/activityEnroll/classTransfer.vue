                ...
                <el-table-column label="操作" width="300" align="center">
                    <template slot-scope="scope">
                        <div v-if="scope.row.hasList>0">
                            <div v-if="scope.row.payable >= 0 && scope.row.status == 0">
                                <el-button type="text">退/付款</el-button>
                                <el-button type="text">不通过</el-button>
                            </div>
                             <div v-if="scope.row.payable >= 0 && scope.row.status == 1">
                                <el-button type="text">需要分班</el-button>
                            </div>
                        </div>
                        <div v-else>
                            <div v-if="scope.row.payable > 0 && scope.row.status == 0">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getPageInfo() {
                let p = {};
                p.sql = `select a.id,a.activityEnrollId,a.toActivityClassId,a.status,
                a.fromFee,a.toFee,a.payable,a.payStatus,b.className as fromClassName,
                c.className as toClassName,d.studentName,e.realName from classTransfer as a
                left join (select m.id,n.className,m.studentId,n.activityClassMainId from activityEnroll as m 
