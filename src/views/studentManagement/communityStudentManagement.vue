<template>
    <div class="articleList">
            ...
            class="table" ref="multipleTable">
                ...
                <el-table-column label="状态" align="center">
                    <template slot-scope="scope">
                        <el-button type="text">已注册</el-button>
                    </template>
                </el-table-column>
        ...
        <!-- 添加 -->
        <studentAdd :addShow="addShow" :sourceType="sourceType" @handCancel="handCancel" @getPageInfo="getPageInfo"/>
        <!-- 修改 -->
        <studentEdit :editShow="editShow" :id="id" @handCancel="handCancel" @getPageInfo="getPageInfo"/>
    </div>

</template>

<script>
    import studentAdd from './studentForm/studentAdd'
    import studentEdit from './studentForm/studentEdit'
    ...
    export default {
        ...
        methods: {
            ...
            getPageInfo() {
                ...
                if(this.whereStr !=""){
                    p.whereStr = `where sourceType = 1${this.whereStr}`;
                }else{
                    p.whereStr = "where sourceType = 1";
                }
                p.orderStr = "order by status asc";
                console.log(p);  
            ...
            deleteConfirm(id) {
                this.$confirm('此操作将永久删除, 是否继续?', '提示', {
                    ...
                    let p = {};
                    p.ids = id;
                    this.api.deleteByIds('student',p)
                    .then(res => {
                        this.$message({
                            type: 'success',
                            message: '删除成功!',
