    ...
    <div class="articleList">
        <div class="container">
            ...
            class="table" ref="multipleTable">
                ...
                <el-table-column prop="bName" label="退款发起人" align="center" :formatter="isBname">
                </el-table-column>
                ...
                <el-table-column prop="payable" label="退款金额" align="center">
                </el-table-column>
                ...
                <el-table-column label="操作" width="150" align="center">
                    <template slot-scope="scope">
                        <el-button type="text" v-if="scope.row.status == 0">同意退款</el-button>
                        <el-button type="text" v-else-if="scope.row.status == 1">已办结</el-button>
                        <el-button type="text" v-else>无</el-button>
                    </template>
                    <div></div>
                </el-table-column>
            </el-table>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getPageInfo() {
                let p = {};
                p.sql = `select a.*,b.realName as bName,d.className,e.studentName,
                e.schoolName,e.gradeName,e.classNumber,f.realName as fName,
                g.actualMoney from classRetire as a 
                left join user as b on a.userId = b.id 
                left join activityEnroll as c on a.activityEnrollId = c.id 
                left join activityClass as d on c.activityClassId = d.id 
                left join student as e on c.studentId = e.id 
                left join admin as f on a.adminId = f.id 
                left join userOrder as g on c.userOrderId = g.id`;
                p.everyPage = this.page.pageSize;
                p.currentPage = this.page.currentPage;
                if(this.whereStr !=""){
                    p.whereStr = `where c.isDeleted = 0${this.whereStr}`;
                }else{
                    p.whereStr = "where c.isDeleted = 0";
                }
                p.orderStr = "order by a.status asc,a.createTime desc";
                console.log(p);  
                this.api.getPageInfo("classRetire",p)
                .then(result => {
                    console.log(result);
            ...
            getListByPage() {
                ...
                p.orderStr = "order by a.status asc,a.createTime desc";
                //console.log(p);  
                this.api.getDataByPage("classRetire",p)
                .then(result => {
                    this.loading = false
                    console.log(result);
